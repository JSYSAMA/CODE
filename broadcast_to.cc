...
bool BroadcastToTiling(const std::string& op_type, const TeOpParas& op_paras, const nlohmann::json& op_info,
    ...
    CHECK(count, "op [%s]: input shape shape cannot be empty", op_type.c_str());
    ...
    for (uint64_t i = 0; i < broadcast_input.size(); i++){
        CHECK(((broadcast_input[i] == shape_value[i]) || (broadcast_input[i] == 1)), "op [%s]: shape of var or shape do not meet the requirements", op_type.c_str());
